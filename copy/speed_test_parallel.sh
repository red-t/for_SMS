...
# split input file
time parallel --pipepart -a tmp.sam --block 50M -j48 -q awk '{split($6, a, /[ISH]/); max=0; for(i in a){match(a[i], /([0-9]+$)/, b); if(b[1]>max){max=b[1]}}; if(max>=200){print $0}}' | cat tmp.header - | samtools view -@ 48 -bhS - | samtools sort -@ 48 -o ${1%.bam}_filtered.bam - && samtools index -@ 48 ${1%.bam}_filtered.bam
