...
parser.add_argument("--fasta-prefix", type=str, required=True, dest="fastaprefix", default=None, help="prefix of the output fasta files")
parser.add_argument("--haploid",action="store_true", dest="haploid", help="Switch to haploid genomes, per default diploid genomes are used; default=False")



args = parser.parse_args()

print "Simulating PacBio reads for individuals"
...
fawriter=fastaIO.FastaBatchWriter(args.fastaprefix,args.haploid,60)
...
for header,seq in fastaIO.FastaReader(args.pop_gen):
    ...
    for i in range(0,targetreads):
        ...
        fawriter.write(h,read1,counter)
        readcount+=1
