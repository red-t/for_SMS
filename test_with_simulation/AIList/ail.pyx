...
cdef class ailist:
    ...
    def dist_p(self, str ctg, uint32_t rpos, uint32_t flank):
        cdef uint32_t n
        cdef uint32_t dist
        n = query_dist_p(self.ail, str.encode(ctg), rpos, flank, &dist)
        return n, dist
    
    def dist_c(self, str ctg, uint32_t st, uint32_t ed, uint32_t flank):
        cdef uint32_t n
        cdef uint32_t dist
        n = query_dist_c(self.ail, str.encode(ctg), st, ed, flank, &dist)
        return n, dist